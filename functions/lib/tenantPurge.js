...
exports.processScheduledDeletions = exports.purgeTenantData = void 0;
 ...
 */
exports.purgeTenantData = functions
    .region('europe-west1')
    .runWith({
    timeoutSeconds: 540,
    ...
    if (!tenantDoc.exists) {
        throw new functions.https.HttpsError('not-found', `El tenant "${targetTenantId}" no existe.`);
    }
    const tenantData = tenantDoc.data();
    // SECURITY: Name must match exactly (prevents copy-paste errors)
    if (tenantData.name !== confirmTenantName) {
        (0, auditLogger_1.logAdminAction)(context.auth.uid, 'PURGE_VERIFICATION_FAILED', targetTenantId, { providedName: confirmTenantName, actualName: tenantData.name }, 'WARNING');
        throw new functions.https.HttpsError('invalid-argument', `El nombre del tenant no coincide. Proporcionado: "${confirmTenantName}", Esperado: "${tenantData.name}"`);
    }
    // ═══════════════════════════════════════════════════════
    ...
    if (softDelete) {
        ...
        (0, auditLogger_1.logAdminAction)(context.auth.uid, 'TENANT_SOFT_DELETE', targetTenantId, {
    ...
    // ═══════════════════════════════════════════════════════
    (0, auditLogger_1.logTenantPurge)(context.auth.uid, targetTenantId, confirmTenantName, auditResults);
    return {
        success: true,
        mode: 'hard_delete',
 ...
 * Run daily at 2 AM
 */
exports.processScheduledDeletions = functions.region('europe-west1').pubsub
    .schedule('0 2 * * *')
    ...
    for (const tenantDoc of pendingDeletions.docs) {
        ...
        if (scheduledDate <= now) {
            ...
            (0, auditLogger_1.logAdminAction)('SYSTEM_SCHEDULER', 'SCHEDULED_PURGE_DUE', tenantDoc.id, { tenantName: data.name, requestedBy: data.deletionRequestedBy }, 'CRITICAL');
        }
    }
});
//# sourceMappingURL=tenantPurge.js.map