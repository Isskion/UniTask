...
console.log('Sheets con "junio":', wb.SheetNames.filter(n => /junio/i.test(n)));
...
function dump(sheetName) {
    ...
    for (let ri = 3; ri < rows.length; ri++) {
        ...
        DAY_OFFSETS.forEach((base, dayIdx) => {
            ...
            if (!actividad) return;
            console.log(`${consultantName} | ${DAY_NAMES[dayIdx]} Fecha_T="${fechaT}" Actividad="${actividad}" Comentario="${comentario}" Horario="${horario}" Resultado="${resultado}"`);
        });
    }
}
