...
export function usePermissions() {
    ...
    const [tenantConfig, setTenantConfig] = useState<{ unitaskToolsEnabled: boolean; enabledTools: string[] }>({
        ...
        if (tenantId) {
            unsubTenant = onSnapshot(doc(db, "tenants", tenantId), (snap) => {
                if (snap.exists()) {
                    ...
                    setTenantAIEnabled(data.aiEnabled !== false);
                    setTenantConfig({
                        unitaskToolsEnabled: data.unitaskToolsEnabled || false,
    ...
    const can = (action: string, context: string): boolean => {
        ...
        // 1. Super Admin Bypass (Absolute Power)
        if (userRole === 'app_admin' || userRole === 'superadmin') return true;

        // 2. View Access Logic (The "Solid" Parameterization)
        if (context === 'views') {
            ...
            const tenantHasTool = isTechnicalTool ? tenantConfig.unitaskToolsEnabled : 
                                 isSpecificTool ? tenantConfig.enabledTools.includes(action) : true;

            if (tenantHasTool) {
                // Rule: Admin, PM, and Consultant ALWAYS see active tools
                const roleLevel = getRoleLevel(userRole);
                if (roleLevel >= RoleLevel.CONSULTANT) return true;
                
                // Rule: Team / Client / Others fallback to their assigned Permission Group (e.g. UniLeaks only)
                return permissions.viewAccess?.[action as keyof typeof permissions.viewAccess] || false;
            }
            return false;
        }

        // 3. Entity & Special Permissions (CRUD / Trash / Technical Info)
        if (context === 'tasks') {
        ...
        if (context === 'project' || context === 'projects') {
