...
export default function LayoutExporter({ isOpen, onClose, initialMode = 'export' }: Props) {
    ...
    const handleExport = useCallback(() => {
        ...
        if (multiSheet.enabled && multiSheet.config.mainSheet) {
            metaRows.push([]);
            metaRows.push(['__MULTI_SHEET_CONFIG__']);
            metaRows.push(['mainSheet', multiSheet.config.mainSheet]);
            metaRows.push(['mainKey', multiSheet.config.mainKey]);
            for (const rel of multiSheet.config.relations) {
    ...
    }, [mappedEntries, rows, mapping, booleanOverrides, dynamicFieldCounts, multiSheet, templateName, includeSampleData, includeFieldPaths]);
