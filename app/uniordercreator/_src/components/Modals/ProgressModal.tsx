...
export default function ProgressModal({
    ...
    // por fila (miles de veces en total) y una animación smooth en cada una satura el hilo
    // principal — visualmente indistinguible aquí, pero sin el costo.
    useEffect(() => {
        logsEndRef.current?.scrollIntoView({ behavior: 'auto' });
    }, [logs]);

    // Techo de líneas realmente montadas en el DOM. `logs` completo (sin techo) se sigue
    // usando para "Copiar errores"/export y queda íntegro en memoria — aquí solo se limita el
    // render. Antes se montaban TODAS las líneas acumuladas sin virtualizar: en un envío de
    // miles de filas esto era miles de nodos DOM re-renderizados en cada línea nueva.
