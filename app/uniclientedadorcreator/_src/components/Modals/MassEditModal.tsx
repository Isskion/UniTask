...
export default function MassEditModal({ isOpen, onClose }: MassEditModalProps) {
    ...
    // Indicador de "trabajando": se marca ANTES del trabajo pesado y se libera en el frame
    // siguiente (requestAnimationFrame), para que React alcance a pintar el spinner antes de
    // que el hilo principal se ocupe con applyBulkEdit — si ambas cosas fueran síncronas en el
    // mismo tick, el usuario nunca vería el indicador (se pintaría todo junto al final).
    const [busy, setBusy] = useState(false);

    if (!isOpen) return null;

    // applyBulkEdit hace UNA sola pasada sobre `rows` y UNA sola notificación de estado —
    // antes se llamaba a updateRowData por cada fila seleccionada (clonaba el array completo
    // en cada llamada; con selección grande, O(n²) sin ningún indicador de progreso).
    const handleApply = () => {
