...
export interface AppState {
   ...
   * con selección grande es O(n²) y sin indicador visual de progreso). */
...
export const useAppStore = create<AppState>((set) => ({
  ...
  currentTab: 'pTransporte', searchQuery: '',
  dynamicFieldCounts: { Transporte: 0 },
  currentLanguage: 'es',
  ...
  clearAllData: () => set((state) => ({
    rows: [], headers: [], selectedIndices: new Set<number>(), selectedRow: -1,
    mapping: {}, booleanOverrides: {}, currentTab: 'pTransporte', searchQuery: '',
    dynamicFieldCounts: { Transporte: 0 },
