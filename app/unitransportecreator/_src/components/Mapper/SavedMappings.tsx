...
export default function SavedMappings({ isOpen, onClose, initialMode = 'list', tenantId }: Props) {
    ...
    const refresh = useCallback(async () => {
        ...
        setLoading(true);
        try {
            // La colección Firestore 'unigis_templates' es compartida por las 5 herramientas
            // Uni[Entidad]Creator y se filtra solo por tenantId (sin moduleId/toolId) — sin este
            // filtro local, aparecerían aquí mezcladas plantillas de Clientes Dadores, Vehículos,
            // etc. cuyas claves no aplican a este schema. Se filtra por prefijo de campo real de
            // esta herramienta en vez de tocar el servicio compartido (usado en producción por
            // las otras 4 herramientas).
            const list = (await loadTemplates(tenantId)).filter((tpl) =>
                Object.keys(tpl.mapping || {}).some((k) => k.startsWith('Root.Transporte.'))
            );
            setTemplates(list);
            setFeedback(null);
        } catch (err: any) {
