...
export default function ResultsDashboard({ isOpen, onClose, onRetryRow, onRetryAll }: ResultsDashboardProps) {
    ...
    // aparecía justo ahí, no durante el envío.
    const stats = useMemo(() => {
    ...
    const handleCopyReport = useCallback(() => {
        ...
        const lines: string[] = [
            `==================================================`,
            `  REPORTE DE RESULTADOS - UNITRANSPORTECREATOR  `,
            `  ${new Date().toLocaleString()}                  `,
            `==================================================`,
            ``,
            `Total Transportes: ${stats.total}`,
    ...
    const handleExportExcel = useCallback(() => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Errores_Transporte');
        XLSX.writeFile(wb, `Errores_Transporte_${Date.now()}.xlsx`);
    }, [stats]);
                ...
                <div className="px-6 py-4 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border-b border-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400">Resultados de integración de Transportes en UNIGIS</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    {activeTab === 'success' && (
                        <div className="space-y-2 max-h-[60vh] overflow-auto pr-1">
                            ...
                            {stats.successRows.length === 0 && (
                                <div className="text-center py-8 text-slate-400">
                                    <div className="text-3xl mb-2 opacity-40">📦</div>
                                    <p className="text-sm font-semibold">No hay transportes exitosos aún</p>
                                </div>
                            )}
                        </div>
                    )}
