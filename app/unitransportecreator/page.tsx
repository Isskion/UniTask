...
import { useAppStore } from '@/app/unitransportecreator/_src/store/appStore';
import { parseExcelFile } from '@/app/unitransportecreator/_src/utils/excelParser';
import { generateValidationReport, type ValidationReport } from '@/app/unitransportecreator/_src/utils/validation';
import { buildXml, type BuildXmlContext } from '@/app/unitransportecreator/_src/services/xmlBuilder';
import { type ProgressLog } from '@/app/unitransportecreator/_src/components/Modals/ProgressModal';
import { postSoapProxy } from '@/lib/soapProxy';

import Header from '@/app/unitransportecreator/_src/components/Header/Header';
import MasterTable from '@/app/unitransportecreator/_src/components/DataPanel/MasterTable';
import DetailPanel from '@/app/unitransportecreator/_src/components/DataPanel/DetailPanel';
import XmlPreview from '@/app/unitransportecreator/_src/components/XmlPreview/XmlPreview';
import MapperPanel from '@/app/unitransportecreator/_src/components/Mapper/MapperPanel';
import LoginModal from '@/app/unitransportecreator/_src/components/Modals/LoginModal';
import ProgressModal from '@/app/unitransportecreator/_src/components/Modals/ProgressModal';
import ValidationReportModal from '@/app/unitransportecreator/_src/components/Modals/ValidationReportModal';
import MassEditModal from '@/app/unitransportecreator/_src/components/Modals/MassEditModal';
import MappingWizard from '@/app/unitransportecreator/_src/components/Wizards/MappingWizard';
import MappingActions from '@/app/unitransportecreator/_src/components/Mapper/MappingActions';
import LayoutExporter from '@/app/unitransportecreator/_src/components/Mapper/LayoutExporter';
import DataPrepModal from '@/app/unitransportecreator/_src/components/Modals/DataPrepModal';
import HelpModal from '@/app/unitransportecreator/_src/components/Modals/HelpModal';
import ResultsDashboard from '@/app/unitransportecreator/_src/components/Dashboard/ResultsDashboard';
import { ToastProvider } from '@/app/unitransportecreator/_src/components/UI/ToastProvider';

import '@/app/unitransportecreator/_src/i18n';
import '@/app/unitransportecreator/_src/App.css';

const SESSION_KEY = 'utrc_session';
const SOAP_ACTION = 'http://unisolutions.com.ar/CrearTransportes';
...
const SENT_CUITS_KEY = 'utrc_sent_cuits';
...
function UniTransporteCreatorPageInner({ tenantId }: { tenantId: string }) {
    ...
    const sendBatch = useCallback(async (batch: { row: any; index: number }[]) => {
        ...
        for (let i = 0; i < batch.length; i++) {
            ...
            const refCol = mapping['Root.Transporte.Referencia'] || mapping['Root.Transporte.RazonSocial'];
            ...
            try {
                ...
                for (let retry = 0; retry <= 2; retry++) {
                    try {
                        if (isDryRun) {
                            ...
                            res = { json: async () => ({ ok: true, status: 200, text: `<Envelope><Body><CrearTransportesResult>true</CrearTransportesResult></Body></Envelope>` }) };
                        } else {
                ...
                // Nombres de etiqueta sin verificar contra una respuesta real de UNIGIS todavía
                // (a diferencia de Clientes Dadores, donde se tuvo que ajustar tras ver falsos
                // positivos con datos reales — ver [[project_unitask_creator_tools_pattern]]).
                // Si el primer envío real no reconoce el resultado, mirar "Ver XML" en el
                // Dashboard y agregar aquí el tag real que devuelva UNIGIS.
                const resultNode =
                    doc.getElementsByTagName('CrearTransportesResult')[0] ||
                    doc.getElementsByTagName('unis:CrearTransportesResult')[0] ||
                ...
                if (isSuccess) {
                    ...
                    logs.push({ ref, status: 'success', msg: `Transporte creado (${resultText || 'OK'})` });
    ...
    const handleSendRemainingUnique = useCallback(() => {
        const cuitCol = mapping['Root.Transporte.Cuit'];
        if (!cuitCol) {
            alert('La columna CUIT no está mapeada — no se puede deduplicar sin ella. Mapéala en el panel "Transporte" antes de usar este botón.');
            return;
        }
            ...
            <div className="flex flex-1 overflow-hidden p-2 gap-0" style={{ paddingBottom: 0 }}>
                <div className="flex flex-col bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden" style={{ width: `${leftWidth}%` }}>
                    <div className="flex justify-between items-center px-2 py-1 border-b border-slate-100 bg-slate-50/50">
                        <span className="text-xs font-semibold text-slate-700">🚚 Transportes</span>
                        <div className="flex gap-1 items-center">
                            ...
                            {/* Sin botón "Campos Dinámicos": CrearTransportes no tiene bloque CampoDinamico en el WSDL, a diferencia de Depositos/ClienteDador — ver DYNAMIC_FIELD_SECTIONS vacío en schema.ts */}
