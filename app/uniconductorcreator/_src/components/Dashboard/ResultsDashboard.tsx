...
export default function ResultsDashboard({ isOpen, onClose, onRetryRow, onRetryAll }: ResultsDashboardProps) {
    ...
    // aparecía justo ahí, no durante el envío. Ver [[project_uniconductor]].
    const stats = useMemo(() => {
    ...
    const handleExportExcel = useCallback(() => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Errores_Conductor');
        XLSX.writeFile(wb, `Errores_Conductor_${Date.now()}.xlsx`);
    }, [stats]);
                ...
                <div className="px-6 py-4 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border-b border-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400">Resultados de integración de Conductores en UNIGIS</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
