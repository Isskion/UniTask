...
export interface AppState {
   ...
   * con selección grande es O(n²) y sin indicador visual de progreso). Ver [[project_uniconductor]]. */
...
export const useAppStore = create<AppState>((set) => ({
  ...
  currentTab: 'pConductor', searchQuery: '',
  dynamicFieldCounts: { Conductor: 0 },
  currentLanguage: 'es',
  ...
  clearAllData: () => set((state) => ({
    rows: [], headers: [], selectedIndices: new Set<number>(), selectedRow: -1,
    mapping: {}, booleanOverrides: {}, currentTab: 'pConductor', searchQuery: '',
    dynamicFieldCounts: { Conductor: 0 },
