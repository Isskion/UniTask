...
function getLeafValue(row: Record<string, any>, fieldPath: string, tag: string, ctx: BuildXmlContext): string {
  ...
  const isDateField = /(Fecha|Vigencia|Datetime|Expedicion|Vencimiento|Nacimiento)/i.test(tag) && !isTimeField;
  if (isTimeField) return String(excelTimeToHHMM(cellValue));

  // CrearConductores exige datetime completo (confirmado contra el XML de ejemplo real:
  // "2015-06-01T00:00:00", no solo "2015-06-01") para Expedicion/Vencimiento — a diferencia
  // de otras herramientas de esta familia donde solo se envía la fecha. Se añade el sufijo
  // aquí, en esta copia de xmlBuilder.ts, sin tocar el formato compartido de dateHelpers.ts.
  const withMidnight = (isoDate: string) => (isoDate ? `${isoDate}T00:00:00` : '');

  if (cellValue instanceof Date) {
    const y = cellValue.getFullYear(), m = String(cellValue.getMonth() + 1).padStart(2, '0'), d = String(cellValue.getDate()).padStart(2, '0');
    return isDateField ? withMidnight(`${y}-${m}-${d}`) : `${y}-${m}-${d}`;
  }
  if (isDateField && typeof cellValue === 'number') {
    try { return cellValue === 0 ? '' : withMidnight(excelSerialToISO(cellValue)); } catch { return String(cellValue); }
  }
  if (isDateField) return withMidnight(formatToUnigisDate(cellValue));
...
export function buildXml(row: Record<string, any>, ctx: BuildXmlContext): string {
  const conductorSchema = (SCHEMA.Root as any).Conductor;
  const conductorXml = buildNode(conductorSchema, 'Root.Conductor', 4, row, ctx);

  return `<?xml version="1.0" encoding="utf-8"?>
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:unis="http://unisolutions.com.ar/">
  <soapenv:Header/>
  <soapenv:Body>
    <unis:CrearConductores>
      <unis:apiKey>${ctx.token || 'TOKEN'}</unis:apiKey>
      <unis:conductores>
        <unis:pConductor>
${conductorXml}        </unis:pConductor>
      </unis:conductores>
    </unis:CrearConductores>
  </soapenv:Body>
</soapenv:Envelope>`;
