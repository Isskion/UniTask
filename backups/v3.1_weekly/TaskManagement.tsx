...
import { collection, getDocs, doc, updateDoc, addDoc, query, orderBy, serverTimestamp, deleteDoc } from "firebase/firestore";
import { useAuth } from "@/context/AuthContext";
import { Loader2, Plus, Edit2, Save, XCircle, Search, Trash2, CheckSquare, ListTodo, AlertTriangle, ArrowLeft, LayoutTemplate, Calendar as CalendarIcon, Link as LinkIcon, Users, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, X, User as UserIcon } from "lucide-react";
...
export default function TaskManagement() {
    ...
    // --- RENDER ---
    if (userRole !== 'app_admin' && userRole !== 'global_pm') return <div className="p-10 text-center text-zinc-500">Acceso restringido.</div>;
            ...
            <div className={cn("w-72 border-r border-white/5 flex-shrink-0 transition-all duration-300", selectedTask ? "hidden lg:block lg:w-72" : "w-full lg:w-72")}>
                <div className="h-full flex flex-col bg-[#09090b]">
                    <div className="p-4 border-b border-white/5 flex justify-between items-center bg-[#0c0c0e]">
                        <h2 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Tareas ({tasks.length})</h2>
                                        ...
                                        {isStatusOpen && (
                                            <>
                                                ...
                                                <div className="absolute right-0 top-full mt-1 w-40 bg-[#18181b] border border-white/10 rounded-lg shadow-2xl z-50 overflow-hidden py-1">
                                                    {(['pending', 'in_progress', 'review', 'completed', 'blocked'] as const).map(s => (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                                ...
                                <div className="md:col-span-5 space-y-6">
                                                                    ...
                                                                    {users.map(u => {
                                                                        ...
                                                                        return (
                                                                            <button
                                                                                key={u.uid}
                                                                                onClick={() => {
                                                                                    const newRaci = { ...formData.raci };
                                                                                    const current = newRaci[role] || [];
                                                                                    if (isAssigned) {
                                                                                        newRaci[role] = current.filter(id => id !== u.uid);
                                                                                    } else {
                                                                                        newRaci[role] = [...current, u.uid];
                                                                                    }
                                    ...
                                    <div className="pt-2 border-t border-white/5 flex flex-col gap-3">
                                        ...
                                        {!isNew && <button onClick={handleDelete} className="w-full py-3 bg-transparent border border-white/10 text-red-400 hover:bg-red-500/10 hover:border-red-500/20 font-bold rounded-lg transition-all text-xs uppercase tracking-wide">Eliminar Tarea</button>}
                                    </div>
                                </div>
                            </div>
                        </div>
