...
async function repairMasterState() {
    ...
    try {
        ...
        let user;
        try {
            user = await auth.getUser(TARGET_UID);
            console.log('✅ Auth user found:', user.uid);
        } catch (e) {
            console.error('❌ Could not find user by UID in Auth. Please check TARGET_UID.');
            return;
        }
...
repairMasterState().catch(console.error).finally(() => process.exit());
