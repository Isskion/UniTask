...
import { Loader2, Save, Calendar, History, CheckCircle2, Plus, X, Layout, Search, Menu, Trash2, Users, RotateCcw, Sparkles, FolderGit2, Wand2, XCircle, ArrowRight, ListTodo, BarChart3, ChevronLeft } from "lucide-react";
...
export default function WeeklyEditor() {
    ...
    const loadData = useCallback(async (dateObj: Date) => {
        ...
        try {
            ...
            if (existing) {
                console.log(`[LoadData] Found existing entry for ${id}`);
                ...
                console.log(`[LoadData] No existing entry for ${id}, creating new blank.`);
                setEntry({
                    id,
                    weekNumber: weekNum,
                    year: yearNum,
    ...
    const handleViewSwitch = (mode: 'editor' | 'dashboard' | 'projects' | 'users' | 'trash' | 'tasks') => {
        ...
        if (mode === 'dashboard') url.searchParams.set('view', 'dashboard');
        else if (mode === 'tasks') url.searchParams.set('view', 'tasks');
    ...
    useEffect(() => {
        console.log("[Effect] Date changed to:", currentDate);
    ...
    useEffect(() => {
        const handleSwitchProject = (e: CustomEvent) => {
            ...
            console.log("[Command K] Switching to:", p.name);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Phase 2: Shadow Writes (Fire & Forget or Await)
            // We await it to ensure data consistency during this critical migration phase
            try {
                await syncShadowProjects(entry);
                console.log("✅ Shadow Sync completed");
            } catch (shadowError) {
                ...
                <div className="flex-1 flex flex-col min-w-0 h-full relative bg-[#0c0c0e] rounded-xl border border-white/5 overflow-hidden shadow-2xl">
                    ...
                    {viewMode === 'editor' && (
                        <div className="h-full flex flex-col">
                            ...
                            {/* Editor Tabs (Project Tabs) */}
                            <div className="flex items-center gap-1 p-2 border-b border-white/5 overflow-x-auto custom-scrollbar bg-black/20">
                                <button
                                    onClick={() => setActiveTab("General")}
                                    className={cn(
                                        "flex items-center gap-2 px-3 py-1.5 rounded text-xs font-medium transition-all shrink-0",
                                        activeTab === "General"
                                    ...
                                    {activeTab === 'General' && (
                                        <>
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="border border-zinc-800 rounded-xl overflow-hidden shadow-sm bg-[#0a0a0a] min-h-[300px]">
                                                    <RichTextEditor
                                                        ...
                                                        placeholder="Write a high-level summary of the week..."
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                {/* CONCLUSIONS */}
                                                <div className="space-y-4">
                                                    <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Key Conclusions</label>
                                                    <textarea
                                                        value={getCurrentData().conclusions}
                                                        onChange={(e) => updateCurrentData("conclusions", e.target.value)}
                                                        className="w-full h-40 bg-[#0a0a0a] border border-zinc-800 rounded-xl p-4 text-sm text-zinc-300 focus:outline-none focus:border-zinc-700 resize-none shadow-inner"
                                                        placeholder="- Point 1..."
                                                    />
                                                </div>
                                                ...
                                                <div className="space-y-4">
                                                    <div className="flex justify-between items-center">
                                                        <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Next Steps</label>
                                                        {previousContextText && (
                                            ...
                                            return (
                                                <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                                                    ...
                                                    <div className="space-y-4 mb-8">
                                                        <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                                                            Project Update: <span className="text-white">{activeTab}</span>
                                                        </label>
