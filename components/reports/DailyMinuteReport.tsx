...
import { JournalEntry, Task, Project } from '@/types';
...
export function DailyMinuteReport({ date, journalEntry, tasks, projects, tenantName }: DailyMinuteReportProps) {
    ...
    const activeProjectIds = new Set(journalEntry.projects?.map(p => p.projectId || ''));

    // Sort projects: Active in journal first, then others if they have critical tasks? 
    // Actually, only show projects MENTIONED in the journal for the "Minute"
    ...
    return (
        <div className="bg-white p-8 max-w-[210mm] mx-auto min-h-screen text-slate-900 font-sans">
            ...
            <div className="border-b-2 border-slate-900 pb-6 mb-8 flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-bold uppercase tracking-tight text-slate-900 mb-2">Minuta Diaria</h1>
                ...
                <div className="text-right">
                    ...
                    <div className="text-sm font-bold uppercase text-slate-500">{format(date, 'MMMM yyyy', { locale: es })}</div>
                </div>
            </div>

            {/* Global Context Section */}
            {(journalEntry.generalNotes) && (
                <div className="mb-8">
                    <h2 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3 border-b border-slate-100 pb-1">Contexto General</h2>
                    <div className="text-sm leading-relaxed text-slate-700 whitespace-pre-wrap bg-slate-50 p-4 rounded-lg border border-slate-100">
                        {journalEntry.generalNotes || "Sin notas generales registrads."}
                    </div>
                </div>
            )}
            ...
            <div className="space-y-8">
                {relevantProjects.map(project => {
                    const entry = journalEntry.projects?.find(p => p.projectId === project.id);
                    const projectTasks = tasksByProject[project.id] || [];
                    const blockers = projectTasks.filter(t => t.isBlocking && t.status !== 'completed');
                    const doneToday = projectTasks.filter(t => t.status === 'completed'); // Need stricter check for "Completed TODAY" logic usually, but for snapshop...
                        ...
                        <div key={project.id} className="break-inside-avoid">
                            ...
                            <div className="grid grid-cols-3 gap-6">
                                ...
                                <div className="col-span-2 space-y-4">
                                    {entry?.pmNotes && (
                                        <div>
                                            <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-1">Avances / Discusión</h4>
                                            <p className="text-xs text-slate-600 leading-relaxed whitespace-pre-wrap">{entry.pmNotes}</p>
                                        </div>
                                    )}
                                    {entry?.conclusions && (
                                        <div>
                                            <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-1">Conclusiones</h4>
                                            <p className="text-xs text-slate-600 leading-relaxed whitespace-pre-wrap italic">{entry.conclusions}</p>
                                        </div>
                                    )}
                                    {entry?.nextSteps && (
                                        <div className="bg-indigo-50 p-3 rounded-lg border border-indigo-100">
                                            <h4 className="text-[10px] font-bold text-indigo-800 uppercase mb-1">Siguientes Pasos</h4>
                                ...
                                <div className="col-span-1 bg-slate-50 rounded-lg p-4 border border-slate-100">
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-3 flex items-center gap-2">
                                        <ListTodo className="w-3 h-3" /> Estado Tareas
                                    </h4>
                                    ...
                                    {projectTasks.length === 0 && (
                                        <div className="text-[10px] text-slate-400 italic">No hay tareas activas reportadas.</div>
                                    )}
                                </div>
                            </div>
            ...
            <div className="mt-12 pt-6 border-t border-slate-200 text-center">
                <p className="text-[10px] text-slate-400 uppercase tracking-widest">Generado automáticamente por UniTask</p>
                <p className="text-[10px] text-slate-300 mt-1">{format(new Date(), 'dd/MM/yyyy HH:mm')}</p>
            </div>
        </div>
    );
}
