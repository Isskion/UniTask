...
import { createTenant } from "@/lib/tenants"; // Import new helper
...
export default function TenantManagement() {
    ...
    const loadTenants = async () => {
        ...
        try {
            ...
            // Sort numeric IDs if needed, but name is usually better for display
    ...
    const handleSave = async () => {
        if (!formData.name) {
            showToast("Error", "Nombre requerido", "error");
            return;
        }
        setProcessing(true);
        ...
        try {
            if (editingTenant) {
                ...
                await createTenant({
                    name: formData.name,
                    code: formData.name.toLowerCase().replace(/\s+/g, '-'), // Auto-slug for code
    ...
    // ALIGN DATA logic remains (simplified)
    const handleAlignData = async () => {
        alert("Esta función está desactivada temporalmente mientras migramos a IDs numéricos.");
    };

    const handleDelete = async (id: string) => {
        if (id === "1") {
            showToast("Error", "No puedes eliminar al Tenant Principal (System).", "error");
            return;
        }
        if (!confirm("¿Eliminar este Tenant? Esto no borra sus datos, pero puede romper referencias.")) return;
        setProcessing(true);
        try {
            ...
            <div className="px-8 py-6 border-b border-border bg-card">
                <div className="flex items-center justify-between max-w-6xl mx-auto">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">Administra organizaciones (IDs Numéricos Secuenciales).</p>
                    </div>
                    <div className="flex gap-2">
                        {/* 
                        <button onClick={handleAlignData} ... > 
                            Alinear Datos 
                        </button> 
                        */}
                        <button
                    ...
                    <div className="bg-card w-full max-w-md rounded-xl shadow-2xl border border-border p-6 space-y-6">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-bold">
                                {editingTenant ? `Editar ${editingTenant.name}` : "Nuevo Tenant"}
                            </h2>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="w-full bg-slate-100 dark:bg-slate-800 border border-border rounded-lg px-3 py-2 text-sm font-mono text-muted-foreground">
                                    {editingTenant ? editingTenant.id : "(Se generará automáticamente: #2, #3...)"}
                                </div>
                            </div>
