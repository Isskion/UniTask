...
export default function ProjectManagement() {
    ...
    // 'editingProject' holds the form data. If null, form is not dirty/active (unless we are in settings view read-only? No, always editable there)
    // Actually, let's use separate state for the Form Data to distinguish "Saved State" (selectedProject) vs "Draft State" (formData)
    ...
    const handleSave = async () => {
        ...
        try {
            if (isNew) {
                ...
                // We need to find the new project to select it. 
                // Since we don't have the object from loadProjects immediately without a refetch or careful merge, 
                // let's just create the object locally to select it immediately for UX
                const createdProject = { id: docRef.id, ...formData } as Project;

                // Update local list strictly for immediate feedback (loadProjects will overwrite eventually)
                setProjects(prev => [...prev.filter(p => p.id !== docRef.id), createdProject].sort((a, b) => a.name.localeCompare(b.name)));

                setSelectedProject(createdProject);
                setIsNew(false);
                setUserTab('feed'); // Go to feed after create? Or stay in settings? Usually Feed is nicer.
                ...
                if (selectedProject?.id) {
                    ...
                    alert("Guardado");
                    // Stay in form or go back? Let's stay in form for updates.
                }
            }
        } catch (e) {
    ...
    if (userRole !== 'app_admin' && userRole !== 'global_pm') {
        return <div className="p-10 text-center text-zinc-500">Acceso restringido.</div>;
    }

    const ProjectList = () => (
        <div className="h-full flex flex-col">
            <div className="p-4 border-b border-white/5 flex justify-between items-center bg-[#0c0c0e]">
                <h2 className="text-sm font-bold text-zinc-400 uppercase tracking-wider">Proyectos ({projects.length})</h2>
                <button
                                    ...
                                    <div className="bg-white/5 border border-white/5 rounded-2xl p-6 space-y-6">
                                        ...
                                        <div className="grid grid-cols-2 gap-6">
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-zinc-200 font-mono focus:border-indigo-500 outline-none uppercase"
                                                    value={formData.code || ""}
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-zinc-200 font-bold focus:border-indigo-500 outline-none"
                                                    value={formData.name || ""}
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-zinc-200 focus:border-indigo-500 outline-none"
                                                ...
                                                <div className="flex gap-2 flex-wrap">
                                                    {["#ef4444", "#f97316", "#f59e0b", "#84cc16", "#10b981", "#06b6d4", "#3b82f6", "#71717a", "#a855f7", "#ec4899"].map(c => (
                                                        <button
                                                            ...
                                                            className={cn("w-6 h-6 rounded-full border-2 transition-transform", formData.color === c ? "border-white scale-110" : "border-transparent opacity-50 hover:opacity-100 hover:scale-110")}
                                                        />
                                                    ))}
                                                </div>
                                            </div>
