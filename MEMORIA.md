...
- **Preservación de API Key en Uni-swagger (Junio 2026)**:
  - **Problema**: El integrador sobrescribía automáticamente el campo `ApiKey` del request body con el token dinámico de sesión (`MapiToken` de login) y lo mandaba en las cabeceras `ApiKey`/`X-ApiKey`. Al consumir endpoints REST como `CrearTipoVehiculo`, el servidor UNIGIS arrojaba `INVALID APIKEY` porque estos servicios requieren la API Key estática original de la empresa en el cuerpo del JSON y en la cabecera de autenticación REST.
  - **Solución**: Se actualizó `injectApiKey` (en `main.js` y `swagger-engine.ts`) para que **no sobrescriba** la API Key si esta ya ha sido ingresada de forma manual en el JSON. Al enviar la petición (`executeServiceCall` y `sendUnitary`/`startMass`), si se detecta una API Key manual en el cuerpo, esta se usa para las cabeceras/URL query de `ApiKey` y `X-ApiKey`, mientras que el token dinámico de login se reserva para `MapiToken`, `Authorization` y `Token`.
